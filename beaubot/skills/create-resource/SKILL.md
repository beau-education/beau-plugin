...
- `showTranscript` (default `true`) controls whether the student is shown the words we heard. **Set it to `false` for speak-then-type items** — a question that asks them to SAY a phrase, followed by one asking them to TYPE the same phrase. Printing the transcription answers the typing half for them. With it off the words stay hidden AND the tutor is instructed not to repeat, spell or hint at them; you still see the transcript in the lesson record.
- **Unless hidden, the student always sees what was transcribed** — before sending in conversation mode, and after submitting in presentation mode. A misheard student can see that's what happened rather than being confused by feedback that doesn't fit their answer. The text is held on a reading-speed estimate from the student's **age**, so an accurate date of birth on the student profile matters here; with none we assume 10.
