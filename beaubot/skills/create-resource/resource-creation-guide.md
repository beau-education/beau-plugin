...
#### Building a spelling or dictation quiz (step by step)

The student **hears** a word or sentence and **types** it; grading is exact. In the editor:

1. Add an **Open Answer** question and set its input type to **Text**.
2. Turn on **Exact match**, and type the word or sentence — with exactly the capital letters and punctuation you want graded — as the expected answer.
3. On the question's illustration, open the **Audio** tab, type the same word or sentence, choose a voice, and **preview** it (regenerate if a tricky word like "read" or "lead" sounds wrong).

The student then hears the clip with a play button and types what they hear — the text itself is never shown. Exact-match feedback says exactly what was wrong ("Check your capital letters", "You forgot the full stop", "You got 2 letters wrong"). Use **Attempts** (under the question's settings) to control retries; leave it unlimited for practice. For a full **dictation lesson**, repeat per phrase — one audio question each — under headings like "Phrase 1", "Phrase 2", …
